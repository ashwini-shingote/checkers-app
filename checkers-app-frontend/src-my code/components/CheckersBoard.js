...
const CheckersBoard = () => {
  ...
  const checkAdjusentCols = (fromPosition) => {
    ...
    if (
      ...
      let nextLVal = "",
        nextRVal = "";
      if (isValidLeft && isValidLeft[1] === 2 && upRow !== 0) {
        nextLVal = getPiece(
          upRow - 1,
          isValidLeft[0] === "left" && toCheckLeftCol - 1
        );
      ...
      if (isValidRight && isValidRight[1] === 2 && upRow !== 0) {
        nextRVal = getPiece(
          upRow - 1,
          isValidRight[0] === "right" && toCheckRightCol + 1
        );
  ...
  function makeKing(board) {
    // Check the first row for red pieces
    for (let col = 0; col < board[0].length; col++) {
      if (board[0][col].startsWith("r")) {
        board[0][col] = board[0][col] + "K";
      }
    }

    // Check the last row for black pieces
    const lastRowIndex = board.length - 1;
    for (let col = 0; col < board[lastRowIndex].length; col++) {
      if (board[lastRowIndex][col].startsWith("b")) {
        board[lastRowIndex][col] = board[lastRowIndex][col] + "K";
      }
    }
                ...
                {takenByPlayer1.length > 0 && (
                  <ul>
                    {takenByPlayer1.map((taken, tid) => {
                      return (
                        <li key={`t1p${tid}`}>
                          <Image src={red_peice} style={{ width: "50px" }} />
                          {/* {taken} */}
                        </li>
                      );
                    })}
                ...
                {takenByPlayer2.length > 0 && (
                  <ul>
                    {takenByPlayer2.map((taken, tid) => {
                      return (
                        <li key={`t2p${tid}`}>
                          <Image src={black_peice} style={{ width: "50px" }} />
                          {/* {taken} */}
                        </li>
                      );
                    })}
